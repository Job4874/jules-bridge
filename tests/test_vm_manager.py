...
class TestGetLocalMemoryPercent:
    @patch("modules.vm_manager._psutil")
    def test_returns_virtual_memory_percent(self, mock_psutil):
...
class TestBootSecondaryVM:
    @patch("modules.vm_manager._psutil")
    def test_no_action_when_memory_low(self, mock_psutil):
    ...
    @patch("modules.vm_manager._psutil")
    def test_dry_run_when_memory_high(self, mock_psutil):
    ...
    @patch("modules.vm_manager._psutil")
    def test_blocks_live_boot_without_allow_flag(self, mock_psutil):
        mock_psutil.virtual_memory.return_value.percent = 90.0
    ...
    @patch("modules.vm_manager._psutil")
    def test_executes_live_boot_when_allowed(self, mock_psutil, mock_run):
        mock_psutil.virtual_memory.return_value.percent = 90.0
    ...
    @patch("modules.vm_manager.subprocess.run")
    @patch("modules.vm_manager._psutil")
    def test_returns_failed_on_azure_error(self, mock_psutil, mock_run):
        mock_psutil.virtual_memory.return_value.percent = 90.0
