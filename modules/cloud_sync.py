...
def _process_planned_entries(
    status: dict[str, Any], entries: list[dict[str, Any]], planned_entries: list[dict[str, Any]]
) -> tuple[CloudSyncStatusResult, list[dict[str, Any]]]:
    if not planned_entries:
        return CloudSyncStatusResult(status), entries
    ...
    untracked_add = sum(1 for entry in planned_entries if not entry.get("tracked"))
    unstaged_add = sum(1 for entry in planned_entries if entry.get("tracked"))
...
def _determine_packet_state(status: dict[str, Any], blockers: list[str]) -> str:
    if blockers:
        return "blocked"
    if status.get("publish_ready"):
        return "push_ready"
...
def _create_error_packet(
    ...
    if error_type == "sync_status_error":
    ...
    *,
    ...
    try:
        ...
        if status.get("status") == "error":
            return _create_error_packet(generated_at, "sync_status_error", status_snapshot=status)
        ...
        status, entries = _process_planned_entries(status, entries, planned_entries)
        ...
        state = _determine_packet_state(status, blockers)
        ...
        return _create_error_packet(generated_at, "publish_packet_error", error_message=str(exc))
