import os, json, sqlite3, subprocess, sys, time  
...
class UnifiedOperatorStateDB:  
    ...
    def save_checkpoint(self, idempotency_key: str, goal: str, workflow_step: str, status: str = 'in_progress', git_branch: str = '', git_commit: str = '', cloud_session_id: Optional[str] = None, payload: Optional[Dict[str, Any]] = None) -> CheckpointRecord:  
        ...
        with self._get_connection() as conn:  
            conn.execute('''  
                INSERT INTO checkpoints (idempotency_key, goal, workflow_step, status, git_branch, git_commit, cloud_session_id, payload_json, created_at_utc, updated_at_utc)  
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)  
                ON CONFLICT(idempotency_key) DO UPDATE SET status=excluded.status, workflow_step=excluded.workflow_step, payload_json=excluded.payload_json, updated_at_utc=excluded.updated_at_utc  
            ''', (idempotency_key, goal, workflow_step, status, git_branch, git_commit, cloud_session_id, payload_json, now, now))  
            conn.commit()  
...
class WorktreeLockManager:  
    ...
    def acquire_lock(self, worktree_path: str, owner_id: str) -> bool:  
        ...
        if norm in self._locks and self._locks[norm] != owner_id: return False  
...
class TaskQueueManager:  
    ...
    def enqueue_task(self, task_id: str, description: str, dependencies: Optional[List[str]] = None, priority: int = 0, idempotency_key: Optional[str] = None, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  
        task = {'task_id': task_id, 'description': description, 'dependencies': dependencies or [], 'status': 'pending', 'priority': priority, 'idempotency_key': idempotency_key or f'task_{task_id}', 'payload': payload or {}, 'created_at_utc': datetime.now(timezone.utc).isoformat()}  
        self.tasks[task_id] = task  
        return task  
    def get_runnable_tasks(self) -> List[Dict[str, Any]]:  
        runnable = []  
        running_count = sum(1 for t in self.tasks.values() if t['status'] == 'running')  
        if running_count  return []  
        for task in sorted(self.tasks.values(), key=lambda x: x['priority'], reverse=True):  
            if task['status'] != 'pending': continue  
            deps_met = all(self.tasks.get(dep_id, {}).get('status') == 'completed' for dep_id in task['dependencies'])  
            if deps_met:  
                runnable.append(task)  
                if len(runnable) + running_count  break  
        return runnable  
    def mark_status(self, task_id: str, status: str) -> None:  
        if task_id in self.tasks: self.tasks[task_id]['status'] = status 
class ObserveReasonActVerifyLoop:  
    ...
    def observe(self, target_dir: Path = _ROOT) -> Dict[str, Any]:  
        ...
        try:  
            git_branch = subprocess.run(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], cwd=target_dir, capture_output=True, text=True).stdout.strip()  
            git_commit = subprocess.run(['git', 'rev-parse', 'HEAD'], cwd=target_dir, capture_output=True, text=True).stdout.strip()  
        except Exception: pass  
        return {'timestamp_utc': datetime.now(timezone.utc).isoformat(), 'git_branch': git_branch, 'git_commit': git_commit, 'python_version': sys.version.split()[0], 'host': os.name}  
...
class ObserveReasonActVerifyLoop:  
    ...
    def reason(self, goal: str, observation: Dict[str, Any]) -> Dict[str, Any]:  
        from modules import reasoning_module  
        res = reasoning_module.plan_only(goal, model='stub')  
        return {'plan': res.get('plan', {}), 'confidence': res.get('confidence', 0.8), 'coherence': True}  
    def act(self, goal: str, step: Dict[str, Any], idempotency_key: str) -> Dict[str, Any]:  
        ...
        self.db.save_checkpoint(idempotency_key=idempotency_key, goal=goal, workflow_step=step.get('description', 'step_action'), status='executing', payload=step)  
        action_result = {'status': 'success', 'step_executed': step.get('description')}  
        self.db.save_checkpoint(idempotency_key=idempotency_key, goal=goal, workflow_step=step.get('description', 'step_action'), status='completed', payload=action_result)  
...
def emit_heartbeat(status: str = 'ok') -> Dict[str, Any]:  
    payload = {'timestamp_utc': datetime.now(timezone.utc).isoformat(), 'timestamp_epoch': time.time(), 'status': status, 'pid': os.getpid(), 'host': os.environ.get('COMPUTERNAME', 'local-host')}  
